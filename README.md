...
The binding will work with and can create snapshots, motion and audio alarms from cameras which only have a RTSP url. 
Some features may not work fully unless your camera also has a snapshot URL, but cameras can come ONLINE without a snapshot url which defaults the binding to using ffmpeg to create snapshots for you.
This uses more CPU to create the snapshots, and you can turn this on and off via the `updateImageNow` channel using a switch or rules. 
You can forget about using the switch if you setup the Config `IMAGE_UPDATE_EVENTS` to be equal 1 and then it runs all the time unless you move the switch to OFF.
Snapshots generated by Ffmpeg use the key frames only, and since come cameras only produce a key frame every 2 seconds or more this will effect how often a snapshot is produced.
Some cameras allow the keyframe to be set by the user, refer to your cameras manual and support.
...
They work with snapshots and mjpeg features, see the examples in the sections below on how to setup these cameras. 
